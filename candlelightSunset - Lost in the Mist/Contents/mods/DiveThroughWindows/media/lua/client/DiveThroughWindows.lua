...
function jumpThroughWindow(character, collider)
    ...
    if instanceof(character, 'IsoPlayer') and character:isSprinting() then
        ...
        if instanceof(collider, 'IsoWindow') or instanceof(collider, 'IsoWindowFrame') then
			...
			if (tostring(playerDir) == 'N' or tostring(playerDir) == 'S') and not collider:getNorth() then
				return
			end
			if (tostring(playerDir) == 'E' or tostring(playerDir) == 'W') and collider:getNorth() then
				return
			end
            ...
            if not collider:isBarricaded() then
				...
				if sprintTime < minRunTime then
					...
					if not collider:isGlassRemoved() and ZombRand(0,math.max(1, BaseDodge + 10*(nimbleLvl + traitBonus + alrDamagedBonus + drunkPenalty) )) <= baseGlassChance then
