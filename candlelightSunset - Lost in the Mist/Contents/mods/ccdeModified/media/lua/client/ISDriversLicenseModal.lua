local ISDriversLicenseModal = ISPanel:derive("ISDriversLicenseModal")
require "lua_timers"

function ISDriversLicenseModal:createChildren()
	local yOffset = 10
	local xPadding = 10
	...
	self.print = ISButton:new(xPadding, yOffset, 150, 25, "PRINT", self, ISDriversLicenseModal.onOptionMouseDown)
	...
	self.cancelButton = ISButton:new(xPadding, yOffset, 150, 25, getText("UI_btn_close"), self, ISDriversLicenseModal.onOptionMouseDown)
...
function ISDriversLicenseModal:onOptionMouseDown(button, x, y)
...
function ISDriversLicenseModal:doPrint()
...
function ISDriversLicenseModal:render()

end

function ISDriversLicenseModal:assignRemovalIndex(index)
	self.removalIndex = index
end

function ISDriversLicenseModal:close()
	self.instance.mainMenu:removeOpenPanel(self.removalIndex, self:toString())
    self:setVisible(false)
    self:removeFromUIManager()
end

function ISDriversLicenseModal:new(x, y, width, height, mainMenu)
	local o = {}
	...
	o.mainMenu = mainMenu
	ISDriversLicenseModal.instance = o
	return o
end

return ISDriversLicenseModal
