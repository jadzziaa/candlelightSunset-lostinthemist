...
function ISVehiclePartMenu.getPropaneTankNotEmpty(playerObj, typeToItem)
	-- Prefer an equipped PetrolCan, then the emptiest PetrolCan, then any PetrolCan.
	local equipped = playerObj:getPrimaryHandItem()
	if equipped and equipped:getType() == "PropaneTank" and equipped:getUsedDelta() > 0 then
		return equipped
	end
...
function ISVehiclePartMenu.onAddPropane(playerObj, part)
	if playerObj:getVehicle() then
		ISVehicleMenu.onExit(playerObj)
	end
	local typeToItem = VehicleUtils.getItems(playerObj:getPlayerNum())
	local item = ISVehiclePartMenu.getPropaneTankNotEmpty(playerObj, typeToItem)
	if item then
