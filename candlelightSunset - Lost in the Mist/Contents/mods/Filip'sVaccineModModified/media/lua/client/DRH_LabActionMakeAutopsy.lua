...
function LabActionMakeAutopsy:perform()	
    ...
    if self.character:getInventory():getItemFromType('AlcoholRippedSheets') then
		inv:Remove('AlcoholRippedSheets');
		inv:AddItem("RippedSheetsDirty");
		self.character:getXp():AddXP(Perks.Doctor, self.top and 45 or 9);
	else--if
		self.character:getXp():AddXP(Perks.Doctor, self.top and 30 or 6);
	end--if	
	if self.corpse then
