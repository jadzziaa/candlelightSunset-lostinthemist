...
function OnEat_EnergyDrink(food, character)
	if DTOverdose then
		DTOverdose.energyDrinkOverdose(character)
	end
end

function OnEat_EnergyDrinkBottle(food, character)
	if DTOverdose then
		DTOverdose.maximumEnergyDrinkOverdose(character)
	end
end

function OnEat_AlcoholMod(food, character)
	if DTOverdose then
...
function OnEat_Tragos(food, character)
	...
	if DTOverdose then
		DTOverdose.tragosOverdose(character)
	end
end

function OnEat_PieQuince(food, character)
	if CMBSpecialLootDef then
		if ZombRand(6) == 2 then
			local plrecipe = character:getKnownRecipes();
			if not plrecipe:contains("FunKey") then
				plrecipe:add("FunKey");
				local key = character:getInventory():AddItem("Base.KeyI");
				key:setKeyId(50010);
				character:Say(getText("Tooltip_BiteKey"));
				character:playSound("DropKey");
			else
				--nothing
			end
		end
...
function BeerPack_TestIsValid(item)
