	...
	if Primary and instanceof(Primary, "HandWeapon") and getCore():isNewReloading() then
		if ISReloadWeaponAction.canShoot(Primary) then
			if Primary:getCondition() > 0 then 
				...
				if Primary:getType() ~= nil then
					if LGHTValidWeapons and LGHTValidWeapons[WType] then
						--local WType = Primary:getType();
						local WeaponSound = Primary:getSwingSound();
						local BaseVolume = getScriptManager():FindItem(LGHTValidWeapons[WType].FullType):getSoundVolume();
						local BaseRadius = getScriptManager():FindItem(LGHTValidWeapons[WType].FullType):getSoundRadius();
						local SoundMod = 1;
						local RadiusMod = 1;
						local Silencer = false;
						if Primary:getCanon() ~= nil and LGHTValidWeapons[WType].Silencers and LGHTValidWeapons[WType].Silencers[Primary:getCanon():getType()] then
							local CanonType = Primary:getCanon():getType();
							if LGHTValidWeapons[WType].Silencers[CanonType].SilencerSoundName then
								WeaponSound = LGHTValidWeapons[WType].Silencers[CanonType].SilencerSoundName;
							end
							if LGHTValidWeapons[WType].Silencers[CanonType].SilencerVolumeMult then
								SoundMod = LGHTValidWeapons[WType].Silencers[CanonType].SilencerVolumeMult;
							end
							if LGHTValidWeapons[WType].Silencers[CanonType].SilencerRadiusMult then
								RadiusMod = LGHTValidWeapons[WType].Silencers[CanonType].SilencerRadiusMult;
							end
						end
						if isDebugEnabled() then
							print("--------------------")
							print("Hunting Test Values:")
							print("--------------------")
							print("Weapon Type: " .. WType)
							print("Weapon Sound Name: " .. WeaponSound)
							print("Base Volume: " .. tostring(BaseVolume))
							print("Base Radius: " .. tostring(BaseRadius))
							print("Sound Volume Multiplier: " .. tostring(SoundMod))
							print("Sound Radius Multiplier: " .. tostring(RadiusMod))
							print("Sound Volume Result: " .. tostring(BaseVolume*SoundMod) .. " (Base Volume* Sound Volume Multiplier)")
							print("Sound Radius Result: " .. tostring(BaseRadius*RadiusMod) .. " (Base Radius* Sound Radius Multiplier)")
						end
						self.character:playSound(WeaponSound);						
						addSound(self.character, self.character:getX(), self.character:getY(), self.character:getZ(), BaseRadius*RadiusMod, BaseVolume*SoundMod);
					end
				end
			end
	...
	if LGHuntingScopeAction.shooted == 1 then
		if self.capture2 ~= "nothing" then
			...
			if (instanceof(Primary, "HandWeapon") and Primary:getCondition() > 0) then
				if Primary:getScope() ~= nil and 
				  LGHTValidWeapons[WType] and 
				  LGHTValidWeapons[WType].ScopeExtraHitChances and
				  LGHTValidWeapons[WType].ScopeExtraHitChances[Primary:getScope():getType()] ~= nil and
				  LGHTValidWeapons[WType].NeedScope then
					local ScopeChance = LGHTValidWeapons[WType].ScopeExtraHitChances[Primary:getScope():getType()]
					if isDebugEnabled() then
						print("Scope Type: " .. Primary:getScope():getType())
						print("Scope Extra Chance: " .. tostring(ScopeChance))
					end
					totalpercent = totalpercent + ScopeChance;
				end
				if not LGHTValidWeapons[WType].NeedScope and LGHTValidWeapons[WType].NoScopeExtraHitChance ~= nil then
					totalpercent = totalpercent + LGHTValidWeapons[WType].NoScopeExtraHitChance;
				end
				if Primary:getCanon() ~= nil and LGHTValidWeapons[WType].Silencers and LGHTValidWeapons[WType].Silencers[Primary:getCanon():getType()] then
					if LGHTValidWeapons[WType].Silencers[Primary:getCanon():getType()].SilencerHitChanceReduction ~= nil then
						local SilencerRedChance = LGHTValidWeapons[WType].Silencers[Primary:getCanon():getType()].SilencerHitChanceReduction;
						if isDebugEnabled() then
							print("Silencer Hit Chance Reduction: -" .. tostring(SilencerRedChance))
						end
						totalpercent = totalpercent - SilencerRedChance;
					end
				end
			end
