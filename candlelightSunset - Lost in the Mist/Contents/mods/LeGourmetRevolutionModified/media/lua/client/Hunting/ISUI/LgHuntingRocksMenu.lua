...
LGHuntingRocksMenu.initContextMenu = function(player, context, worldobjects)
	...
	if htree or htree2 then
		...
		if htree2 and htree2:getSquare():DistToProper(playerObj:getSquare()) > 8 and (instanceof(Item, "HandWeapon") and Item:getCondition() > 0) then
			if LGHTValidWeapons[Item:getType()] then
				...
				tooltipexplore.description = " <RGB:0,0.6,1> " .. getText("ContextMenu_HuntwithRifleTTA") .. " <LINE> <LINE> <RGB:1,1,1> " .. getText("ContextMenu_HuntwithRifleTTB") .. " <LINE> <RGB:0,0.8,0> " .. Item:getDisplayName();
				--Scope
				if LGHTValidWeapons[Item:getType()].NeedScope then
					local Scope = Item:getScope();
					local IsValid = false;
					if Scope ~= nil then
						for i=0, #LGHTValidWeapons[Item:getType()].ValidScopes do
							if Scope:getType() == LGHTValidWeapons[Item:getType()].ValidScopes[i] then
								IsValid = true;
								break;
							end
						end
					...
					if not IsValid then
						...
						tooltipexplore.description = tooltipexplore.description .. " <LINE> <LINE> <RGB:1,1,1> " .. getText("ContextMenu_ScopeType") .. " <RGB:1,0,0> <SPACE> " .. getText("ContextMenu_NeedScope");
					else
						tooltipexplore.description = tooltipexplore.description .. " <LINE> <LINE> <RGB:1,1,1> " .. getText("ContextMenu_ScopeType") .. " <LINE> <RGB:0,0.8,0> " .. Scope:getName() .. LGHuntingRocksMenu.getAttachmentValues(Item, "Scope", Scope:getType());
					end
				else
					tooltipexplore.description = tooltipexplore.description .. " <LINE> <LINE> <RGB:1,1,1> " .. getText("ContextMenu_ScopeType") .. " <LINE> <RGB:0,0.8,0> <SPACE> " .. getText("ContextMenu_NotNeedScope") .. LGHuntingRocksMenu.getAttachmentValues(Item, "NoScope");
				end
				--Silencers
				if LGHTValidWeapons[Item:getType()].Silencers then
					local Silencer = Item:getCanon();
					if Silencer and LGHTValidWeapons[Item:getType()].Silencers and LGHTValidWeapons[Item:getType()].Silencers[Silencer:getType()] ~= nil then
						tooltipexplore.description = tooltipexplore.description .. " <LINE> <LINE> <RGB:1,1,1> " .. getText("ContextMenu_SilencerType") .. " <LINE> <RGB:0,0.8,0> " .. Silencer:getName() .. LGHuntingRocksMenu.getAttachmentValues(Item, "Silencers", Silencer:getType())
					end
				end
				--
				--Zone
				if zonenumber < 100 then
					searchoption.notAvailable = true;
					tooltipexplore.description = tooltipexplore.description .. " <LINE> <LINE> <RGB:1,0,0> ".. getText("ContextMenu_CantHuntwithRifleTT");
				else
				end
				--
...
LGHuntingRocksMenu.getAttachmentValues = function(weapon, attachment, atype)
	local txt = "";
	local wtable = LGHTValidWeapons[weapon:getType()];
	if wtable ~= nil then
		if attachment == "Scope" then
			if wtable.NeedScope then
				--txt = " <LINE> <RGB:1,0.94,0.2> " .. getText("ContextMenu_ScopeProps") .. " <SPACE> ";
				if wtable.ScopeExtraHitChances ~= nil and wtable.ScopeExtraHitChances[atype] ~= nil then
					txt = txt .. " <LINE> <RGB:1,0.94,0.2> " .. getText("ContextMenu_HitChances") .. " <RGB:0,0.8,0> <SPACE> +" .. tostring(wtable.ScopeExtraHitChances[atype]) .."%";
				end
				if wtable.ScopeExtraSpotChance ~= nil and wtable.ScopeExtraSpotChance[atype] ~= nil then
					txt = txt .. " <LINE> <RGB:1,0.94,0.2> " .. getText("ContextMenu_FindChances") .. " <RGB:0,0.8,0> <SPACE> +" .. tostring(wtable.ScopeExtraSpotChance[atype]).."%";
				end
			end
		end
		if attachment == "NoScope" then
			if not wtable.NeedScope and wtable.HaveBaseScope then
				--txt = " <LINE> <RGB:1,0.94,0.2> " .. getText("ContextMenu_ScopeProps") .. " <SPACE> ";
				if wtable.NoScopeExtraHitChance ~= nil then
					txt = txt .. " <LINE> <RGB:1,0.94,0.2> " .. getText("ContextMenu_HitChances") .. " <RGB:0,0.8,0> <SPACE> +" .. tostring(wtable.NoScopeExtraHitChance).."%";
				end
				if wtable.NoScopeExtraSpotChance ~= nil then
					txt = txt .. " <LINE> <RGB:1,0.94,0.2> " .. getText("ContextMenu_FindChances") .. " <RGB:0,0.8,0> <SPACE> +" .. tostring(wtable.NoScopeExtraSpotChance).."%";
				end
			end
		end
		if attachment == "Silencers" then
			--txt = " <LINE> <RGB:1,0.94,0.2> " .. getText("ContextMenu_SilencerProps") .. " <SPACE> ";
			if wtable.Silencers ~= nil and wtable.Silencers[atype] ~= nil and wtable.Silencers[atype].SilencerHitChanceReduction ~= nil then
				txt = txt .. " <LINE> <RGB:1,0.94,0.2> " .. getText("ContextMenu_HitChances") .. " <RGB:1,0,0> <SPACE> -" .. tostring(wtable.Silencers[atype].SilencerHitChanceReduction).."%";
			end
		end
	end
	return txt;
...
LGHuntingRocksMenu.getcapturedata2 = function()
	...
	if (instanceof(Primary, "HandWeapon") and Primary:getCondition() > 0) then 
		if LGHTValidWeapons[Primary:getType()] then
			if Primary:getScope() ~= nil and LGHTValidWeapons[Primary:getType()].ScopeExtraSpotChance and LGHTValidWeapons[Primary:getType()].NeedScope then
				local ExtraChance = 0;
				if LGHTValidWeapons[Primary:getType()].ScopeExtraSpotChance[Primary:getScope():getType()] then
					ExtraChance = LGHTValidWeapons[Primary:getType()].ScopeExtraSpotChance[Primary:getScope():getType()]
				end
				totalpercent = totalpercent + ExtraChance;
			end
			
			if not LGHTValidWeapons[Primary:getType()].NeedScope and LGHTValidWeapons[Primary:getType()].NoScopeExtraSpotChance ~= nil then
				totalpercent = totalpercent + LGHTValidWeapons[Primary:getType()].NoScopeExtraSpotChance;
			end
		end
	end
	if playerObj:HasTrait("ShortSighted") then
