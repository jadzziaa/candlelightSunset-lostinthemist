...
function LGHuntingRocksAction:perform()
	...
	--if LGHuntingRocksAction.shooted == 1 then
		if self.capture ~= "nothing" then
			...
			local perklvlH = self.character:getPerkLevel(Perks.LGRHunting);
			if perklvl > 0 then
				variable = perklvl * 4;
			...
			if ZombRand(1, 101) <= totalpercent then
				...
				if self.capture == "Base.Bag_DuffelBag" then
					...
					if SnakeClothingMod and not recipes:contains("DitoBag") then
						recipes:add("DitoBag");
						self.character:playSound("DropKey");
						local key = self.htree:getSquare():AddWorldInventoryItem("Base.KeyD", self.htree:getX() - math.floor(self.htree:getX()), self.htree:getY() - math.floor(self.htree:getY()), self.htree:getZ() - math.floor(self.htree:getZ()));
						key:setKeyId(50005);
	...
	--				end
				...
				self:AddHuntingxp(perklvlH, (perklvlH +1) * ZombRand(5,11));
			else
