...
ISInjectPlayerTestV1 = ISBaseTimedAction:derive("ISInjectPlayerTestV1");

function ISInjectPlayerTestV1:isValid()
    return self.patientX == self.patient:getX() and self.patientY == self.patient:getY();
end

function ISInjectPlayerTestV1:waitToStart()
...
function ISInjectPlayerTestV1:update()
    if self.patient ~= self.surgeon then
        self.surgeon:faceThisObject(self.patient)
    end
end

function ISInjectPlayerTestV1:start()
    if self.patient == self.surgeon then
...
function ISInjectPlayerTestV1:perform()
    if self.patient ~= self.surgeon and isClient() then
        SendInjectPlayerTestV1(self.patient);
    else
        InjectPlayerTestV1();
    end
    self.surgeon:getXp():AddXP(Perks.Doctor, 2);
    if FIL_isPlayerHaveTestV(true) then
        self.surgeon:getInventory():Remove('CmpSyringeWithTestVaccine');
        self.surgeon:getInventory():AddItem("LabItems.LabSyringeUsed");
    else--if
        if FIL_isPlayerHaveTestVRE(true) then
            self.surgeon:getInventory():Remove('CmpSyringeReusableWithTestVaccine');
...
function ISInjectPlayerTestV1:new(patient, surgeon)
    local o = {}
    setmetatable(o, self)
    self.__index = self
    o.character = surgeon;
